...
var vocabs = [
    ...
    ['dbpedia: <http://dbpedia.org/resource/>', 'dbpedia: <http://dbpedia.org/resource/>']
];
...
Blockly.Blocks['subjectdef'] = {
    init: function() {
        // adding collapse blocks function
        this.appendDummyInput('coll')
            .appendField('Collapse')
            .appendField(new Blockly.FieldCheckbox('FALSE'), 'COLL');
        //---------------------------------
        ...
        this.setPreviousStatement(true, ['subjectdef', 'comment']);
        this.setNextStatement(true, 'subjectdef', 'comment');
        this.setTooltip('Defines a subject definition.');
        this.setInputsInline(true);
    }, onchange: function(event) {
        var blockId = event.blockId;
        if(this.getFieldValue("COLL") == "TRUE") {
            // alert("clicked true");
            workspace.getBlockById(blockId).setCollapsed(true);
            this.setFieldValue("FALSE","COLL");
            // this.getInput('COLL').setVisible(false);

            this.appendDummyInput('coll2')
                .appendField('Expand')
                .appendField(new Blockly.FieldCheckbox('FALSE'), 'COLL2');

        }
        if(this.getFieldValue("COLL2") == "TRUE"){
            this.setFieldValue("FALSE","COLL2");
            workspace.getBlockById(blockId).setCollapsed(false);
            this.removeInput('coll2');
        }
